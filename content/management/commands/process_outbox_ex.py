...
class Command(BaseCommand):
    help = "Process outbox for SMS/Email (EX). Inline SMS; email via services.comms_outbox with optional throttle bypass."

    def add_arguments(self, parser):
        parser.add_argument("--only", choices=["sms", "email", "both"], default="both")
        parser.add_argument("--limit", type=int, default=200)
        parser.add_argument("--ignore-throttle", action="store_true")

    def handle(self, *args, **opts):
        only = opts["only"]
        limit = int(opts["limit"])
        ignore_throttle = bool(opts.get("ignore_throttle", False))
